...
All data stays in glycles.db in the project folder.
"""
import sqlite3
import os
    ...
    with get_conn() as conn:
        conn.executescript("""
            CREATE TABLE IF NOT EXISTS glucose_readings (
                ...
                ts          TEXT NOT NULL UNIQUE,   -- ISO timestamp UTC
                value_mmol  REAL NOT NULL,
                trend       INTEGER,                -- 0-7 LibreLink trend arrow
                source      TEXT DEFAULT 'libre'
            );

            CREATE TABLE IF NOT EXISTS cycles (
                id              INTEGER PRIMARY KEY AUTOINCREMENT,
                period_start    TEXT NOT NULL UNIQUE,  -- ISO date YYYY-MM-DD
                cycle_length    INTEGER DEFAULT 28,    -- days (editable)
                notes           TEXT
            );
...
def upsert_reading(ts: str, value_mmol: float, trend: int = None):
    """Insert a reading; silently skip if timestamp already exists."""
    with get_conn() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO glucose_readings (ts, value_mmol, trend) VALUES (?, ?, ?)",
            (ts, value_mmol, trend)
        )


def get_readings(start: str = None, end: str = None):
    """Return readings as list of dicts, optionally filtered by ISO date range."""
    with get_conn() as conn:
        if start and end:
            rows = conn.execute(
            ...
            rows = conn.execute(
                "SELECT * FROM glucose_readings ORDER BY ts DESC LIMIT 2000"
            ).fetchall()
...
def add_cycle(period_start: str, cycle_length: int = 28, notes: str = ""):
    """Add a period start date. period_start = 'YYYY-MM-DD'."""
    with get_conn() as conn:
        conn.execute(
...
def get_cycles():
    """Return all cycles sorted by date descending."""
    with get_conn() as conn:
        rows = conn.execute(
...
def get_phase_for_date(d: date, cycles: list) -> str:
    """
    Given a date and list of cycle dicts, return the phase name.
    Uses a simple 4-phase model:
      Menstrual:  days 1–5
...
def annotate_readings_with_phase(readings: list, cycles: list) -> list:
    """Add 'phase' and 'phase_color' fields to each reading dict."""
    for r in readings:
...
def compute_phase_stats(readings: list) -> dict:
    """
    Compute per-phase stats: mean, std, time-in-range (3.9–10.0 mmol/L),
    and count. Returns dict keyed by phase name.
    """
    import statistics
...
def compute_cycle_trend(cycles: list, readings: list) -> list:
    """
    For each cycle, compute mean glucose per phase.
    Returns list of dicts: {cycle_label, menstrual, follicular, ovulatory, luteal}
    """
    from datetime import timedelta
    result = []
